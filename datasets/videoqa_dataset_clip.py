...
class VideoQA_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # concat question with answer candidate for all possibilities
        text = [self._get_text(question, x, sub) for x in self.a2id]
...
def build_videoqa_dataset_clip(dataset_name, split, args):
    if dataset_name == "msvd":
        if split == "train":
            csv_path = (
                args.msvd_train_csv_path
                if not args.n_shots
                else args.msvd_train_csv_path.replace(".csv", f"{args.n_shots}p.csv")
            )
        elif split == "val":
        ...
        if split == "train":
            csv_path = (
                args.msrvtt_train_csv_path
                if not args.n_shots
                else args.msrvtt_train_csv_path.replace(".csv", f"{args.n_shots}p.csv")
            )
        elif split == "val":
        ...
        if split == "train":
            csv_path = (
                args.activitynet_train_csv_path
                if not args.n_shots
                else args.activitynet_train_csv_path.replace(
                    ".csv", f"{args.n_shots}p.csv"
                )
            )
        elif split == "val":
        ...
        if split == "train":
            csv_path = (
                args.ivqa_train_csv_path
                if not args.n_shots
                else args.ivqa_train_csv_path.replace(".csv", f"{args.n_shots}p.csv")
            )
        elif split == "val":
        ...
        if split == "train":
            csv_path = (
                args.tgif_frameqa_train_csv_path
                if not args.n_shots
                else args.tgif_frameqa_train_csv_path.replace(
                    ".csv", f"{args.n_shots}p.csv"
                )
            )
        elif split == "val":
        ...
        if split == "train":
            csv_path = (
                args.lsmdc_train_csv_path
                if not args.n_shots
                else args.lsmdc_train_csv_path.replace(".csv", f"{args.n_shots}p.csv")
            )
        elif split == "val":
